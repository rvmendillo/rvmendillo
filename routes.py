    ...
    if request.method == 'POST':
        if name == 'midi_to_relative_scale':
            ...
            if verify_captcha():
                ...
                font_path = 'http://rvmendillo.com/static/fonts/Consolas.TTF'
                return redirect(url_for(name, project=dumps(project), input_type=dumps(input_type), image_path=dumps(image_path), target_width=dumps(target_width), charset=dumps(charset), color_inversion=dumps(color_inversion), output_type=dumps(output_type), font_path=dumps(font_path)), code=302)
            return 'reCAPTCHA validation failed.'
